...
from PyQt5.QtWidgets import QDockWidget, QDialog, QTreeWidgetItem, QTableWidgetItem, QFileDialog, QColorDialog, QWidget, QMenu, QComboBox
...
class LabelDispDock(QDockWidget):
    ...
    def load_labels(self, filename=None):
        if filename is None:
            filename = QFileDialog.getOpenFileName(None, "Select .json/.hdf5 File", directory=self.config['defaultLabelListDir'])[0]
    ...
    def save_labels(self, filename=None):
        if filename is None:
            filename = QFileDialog.getSaveFileName(parent=self, caption='Select Export File...', filter='JSON Files (*.json)', directory=self.config['defaultLabelListDir'])[0]
    ...
    def withdraw(self):
        ...
        self.labelMgr[prop_name][label_name].withdraw(timestamp)
        ...
        if item is not None:
            ...
            if item.type() == PROPERTY:
                ...
                if self.annotation is not None and self.annotation.has(label):
                    sync = True
                self.labelMgr.remove_label(label_name)
