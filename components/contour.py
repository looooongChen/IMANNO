...
def mask2contour(mask):
    ...
    contours = [np.squeeze(cv2.approxPolyDP(c, 0.7, True)) for c in contours if len(c)>=MIN_PTS]
    contours = [c for c in contours if len(c)>=MIN_PTS]
    return contours
    ...
    for idx_query, c_query in enumerate(contour_query):
        for idx_set, c_set in enumerate(contour_set):
            ...
            if len(contour_inter) != 0:
                if len(contour_inter) > 1:
                    contour_inter = [np.squeeze(np.array(c)) for c in contour_inter]
                    area_inter = np.sum([cv2.contourArea(c) for c in contour_inter])
                else:
                    contour_inter = np.squeeze(np.array(contour_inter))
                    area_inter = cv2.contourArea(contour_inter)
                match[idx_query, idx_set] = 2*area_inter/(area_query[idx_query]+area_set[idx_set])
    return match
